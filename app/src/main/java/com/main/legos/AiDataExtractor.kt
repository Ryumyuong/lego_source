...
- "월 순수익 0만"과 "월 소득 60만"이 함께 있으면 → 0이 아닌 값(60) 사용!
  여러 월소득 값이 있으면 0이 아닌 최대값 사용!
- ★ "월 순수익 N만"이 명시되어 있고 0이 아니면 → 순수익 값을 사용! (소득이 더 커도 순수익 우선)
  예: "월 소득 2162만 > 월 순수익 300~400만" → 400 (순수익 최대값)
  예: "월 소득 500만 / 순수익 200만" → 200
- ★ "순수익 적자", "순수익 마이너스", "순수익 적자라고함" 등 적자 표현 → 소득 0
- 3.3% 공제 후 금액 그대로
- 연봉만 있으면 ÷12
...
★★★ 채무현황이 표가 아닌 텍스트 목록일 경우 (예: "24년 3월 하나카드 1250만" 형태):
  - 모든 항목의 금액을 합산! (같은 채권사라도 각 항목 별도 합산!)
  - ★★★ 텍스트 목록의 카드사 항목(예: 하나카드 1250만)과 카드이용금액(예: 하나 1500만)은 별개! 둘 다 합산!
  - targetDebt = 채무현황 텍스트 항목 합계 + 카드이용금액 합계
  - creditors에는 텍스트 항목의 채권사 + 카드이용금액 채권사 모두 포함
...
- ★ 현금서비스(0041) 처리 규칙 (같은 카드사 기준으로 비교!):
  - 각 카드사별로 표의 현금서비스(0041) 합계와 카드이용금액을 비교
  - 카드이용금액 >= 현금서비스 합계 → 카드이용금액만 합산 (현금서비스 제외)
  - 현금서비스 합계 > 카드이용금액 → 현금서비스 + 카드이용금액 둘 다 합산!
  예: 삼성카드 현금서비스(0041) 500만, 삼성카드 카드이용금액 800만 → 800>=500 → 카드이용금액 800만만 합산
  예: KB국민카드 현금서비스(0041) 300만, KB국민카드 카드이용금액 100만 → 300>100 → 현금서비스 300만 + 카드이용금액 100만 = 400만 합산
...
- ★ 제외 코드 목록 (이 코드만 제외!): (200),(230),(270),(290),(400),(500),(3011),(510),(1061),(1421),(1071),(0041)
...
- ★ 민사 소송금 제외
- ★ 통신사(SKT, KT, LG U+ 등) 채무는 대상채무에 포함! 제외하지 않음!
  ★ 기타채무 표에 통신사가 있으면 대상채무에 합산! (예: 기타채무 통신사 kt 20만 → targetDebt에 +20, creditors에 추가)
...
★ 대출과목 유형별 대상채무 제외 규칙:
  ...
  - "신복", "신복위", "신용회복위원회" + "포함" → 대상채무에 포함! (담보 제외 규칙보다 "신복 포함" 우선!)
    예: "신한은행(상가담보/신복 포함ㅇ)" → 담보이지만 "신복 포함" → 대상채무에 포함!
    예: "농협은행(상가담보/신복 포함ㅇ)" → 대상채무에 포함!
    ★ 단, 대출과목 하단의 카테고리 요약 "4. 신복 1억3659만"은 개별 채권사가 아닌 요약이므로 무시!
...
- ★ 단, 같은 카드사에 현금서비스(0041)가 있고 카드이용금액 >= 현금서비스이면 현금서비스 제외! (위 규칙 참조)
- targetDebt = 표 포함항목 합계 + 카드이용금액 (현금서비스 중복 제거 후)
...
3단계: ★★★ 카드이용금액 합산! 현금서비스(0041) 중복 제거 규칙 적용!
  - 카드이용금액은 만원 단위이므로 ×10하여 천원으로 환산
  - 같은 카드사에 현금서비스(0041)가 있으면: 카드이용금액 >= 현금서비스 → 카드이용금액만 (현금서비스 제외)
  - 현금서비스 > 카드이용금액 → 둘 다 합산
  예: 신한카드 현금서비스 2000천원, 신한카드 카드이용금액 400만(=4000천원) → 4000>=2000 → 카드이용금액 4000천원만 합산
  예: KB국민카드 현금서비스 5000천원, KB국민카드 카드이용금액 490만(=4900천원) → 5000>4900 → 둘 다 합산 9900천원
4단계: 최종 합계(천원) ÷ 10 반올림 → 만원
  ★ 이때 표 숫자를 만원으로 착각하지 않도록 주의! 표 66,666은 6667만원!

표가 없을때만 "대출과목" 요약 사용:
  신용 + 카드 + 신복/신복위 합산 (신복도 대상채무에 포함!)
  ★ 반드시 제외: 담보 (단, "신복 포함ㅇ"으로 태깅된 담보는 대상채무에 포함!)
  ★ 학자금(한국장학재단)은 대상채무에 포함!
  ★ "총액"이 있으면 총액 - 순수담보(신복 미포함 담보만)로 검증
  예: 신용=1809만, 카드=2200만, 담보=0만, 총액=4009만 → 4009만
  예: 신용=610만, 신복위=1589만, 담보=0만 → 610+1589 = 2199만 (신복위 포함!)
  예: 신용=0만, 카드=50만, 담보=1억1516만, 신복=1억3659만 → 50+13659 = 1억3709만 (담보 제외, 신복 포함!)
...
- ★ "배우자(랑)공동명의"도 "배우자명의"와 동일하게 ÷2 처리!
  예: "배우자랑공동명의 시세 4억 > 배우자명의대출 1억 / 세입자전세 1억9천" → (40000-10000-19000)÷2 = 5500만
...
- 배우자명의, 배우자공동명의, 부모명의, 형제명의 등 본인명의가 아닌 부동산/보증금의 재산 합계 (만원)
- 배우자명의/배우자공동명의는 ÷2 적용 후 금액
- 본인명의 재산만 있으면 0
- 예: property=10800 중 배우자명의 아파트 2750만 → othersProperty=2750
- 예: 배우자랑공동명의 아파트 (40000-10000-19000)÷2=5500만 → othersProperty=5500
...
E) 대상채무 채권사 목록 (creditors)
   ...
   - ★ "신복 포함ㅇ"으로 태깅된 채무도 creditors에 포함! (대상채무에 포함되므로)
...
F) 담보대출 합계 (damboDebt)
   - 대상채무에서 제외된 담보대출 코드 (200),(230),(270),(290),(220),(1071) 항목의 금액 합계 (만원)
