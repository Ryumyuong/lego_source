...
- excludedCreditors: 각 행에서 추출 (reason 필드는 반드시 포함!)
  ...
  - ★★★ 원금이 0이거나 비어있어도, 제외사유가 "완제"여도 반드시 행 추출 (행 인덱스 일치가 중요)
  - name: 채권금융회사명
  - principal: 원금을 반드시 "원 단위"로 변환한 값 (천원/만원이면 변환 필수). 빈 칸이면 0
...
{"processType": "신/프/워/빈문자열", "processTitle": "제목원문", "totalPrincipal": 숫자, "excludedDebtTotal": 숫자, "defermentMonths": 숫자, "creditors": [{"name": "채권사명", "principal": 숫자}], "excludedCreditors": [{"name": "채권사명", "principal": 숫자, "reason": "사유"}], "outsideCreditors": [{"name": "채권사명", "principal": 숫자}]}"""
        ...
        if (excludedCreditorsArr != null) {
            for (i in 0 until excludedCreditorsArr.length()) {
                ...
                // 완제 행: 인덱스 정렬용으로만 저장 (대출과목 "제외N" 매칭용, 채무 합산은 안 함)
                if (reason.contains("완제")) {
                    if (cName.length >= 2) pdfExcludedEntries.add(PdfExcludedEntry(i + 1, cName, 0, reason, false))
                    Log.d("FILE_PROCESS", "제외 채무 (완제, 합산 제외): 행${i+1} $cName")
                    continue
                }
                if (cPrincipal > 0) {
                    // 현금서비스는 채무에 포함하지 않음
                    ...
                    // 새출발기금 매입예정 → 채권사명만 등록 (대상채무 금액은 totalPrincipal에 이미 반영됨, 중복 합산 금지)
                    if (reason.contains("새출발기금") && reason.contains("매입")) {
                        if (cName.length >= 2 && !pdfAgreementCreditors.containsKey(cName)) {
                            pdfAgreementCreditors[cName] = 0  // 채권사명만 등록 (금액은 totalPrincipal 사용)
                        }
...
{"excludedCreditors": [{"name": "채권사명", "principal": 숫자, "reason": "사유"}]}"""
                ...
                if (retryArr != null && retryArr.length() > 0) {
                    ...
                    for (i in 0 until retryArr.length()) {
                        ...
                        if (reason.contains("완제")) {
                            if (cName.length >= 2) pdfExcludedEntries.add(PdfExcludedEntry(i + 1, cName, 0, reason, false))
                            Log.d("FILE_PROCESS", "제외 채무 (${retryNum}차, 완제): 행${i+1} $cName")
                            continue
                        }
                        if (cPrincipal > 0) {
                            if (reason.contains("현금서비스")) {
                            ...
                            // 새출발기금 매입예정 → 채권사명만 등록 (대상채무 금액은 totalPrincipal에 이미 반영됨)
                            if (reason.contains("새출발기금") && reason.contains("매입")) {
                                if (cName.length >= 2 && !pdfAgreementCreditors.containsKey(cName)) {
                                    pdfAgreementCreditors[cName] = 0
                                }
...
{"totalDebt": 원단위숫자, "totalRepayment": 총변제예정액원단위숫자, "repaymentMonths": 개월수숫자, "creditors": [{"name": "채권자명", "amount": 원단위숫자}, ...]}"""
    ...
    private fun splitMergedTableLines(rawLines: List<String>): List<String> {
        ...
        for (l in rawLines) {
            ...
            // 우측 표가 좌측 표보다 행이 적을 때, 좌측 마지막 행(예: 카드 7행 vs 대출과목 6행 → 마지막 카드 행)은
            // 우측 셀이 비어 탭 개수가 splitIndices[1]까지만 있을 수 있음 → splitIndices[1] 이상이면 수용
            // (flushMergedRows의 start >= parts.size 가드가 부분 행을 안전하게 처리)
            if (splitIndices.size >= 2 && parts.size >= splitIndices[1]) {
                mergedRows.add(parts)
                continue
            }
    ...
    private fun parseHwpData(text: String) {
        ...
        var parsedPensionTotal = 0   // 파싱된 연금 합계 (만원, parsedMonthlyIncome에 포함되어 있음)
        var parsedSudangTotal = 0    // 파싱된 수당 합계 (만원, parsedMonthlyIncome에 포함되어 있음) - 결과창 분해 표시용
        ...
        val paidOffCreditorKeywords = mutableListOf<String>()  // 특이사항/대출과목에서 완납된 채권사 키워드
        val paidOffExcludeSeqs = mutableListOf<Int>()  // 대출과목 "제외N 완납" 행의 N 값 (PDF 제외채무 행번호)
        ...
        val postApplicationDamboCreditors = mutableMapOf<String, Int>()  // 신청일자 이후 담보 채무 (대상채무 미합산, 특이 표기 전용)
        ...
        for (rawLine in lines) {
            ...
            // 채무현황 표(현황순번 채권사명 담보여부) header 감지 → 표 안에 명시된 담보 순번만 담보로 인정
            if (l.contains("현황순번") && l.contains("담보여부")) hasHwangsangSeqHeader = true
            ...
            val damboM = Pattern.compile("((?:\\d{1,2},)*\\d{1,2})(?!년)([^\\d]{2,}?)(?:차량담보|차량할부|차량리스|자동차담보|차담보|집담보|기계담보|중도금대출|중도금|주택담보|부동산담보|아파트담보|보증금담보|주식담보)").matcher(l)
            while (damboM.find()) {
        ...
        fun processHwpLine(rawLine: String) {
            ...
            if (fieldCheckStr.startsWith("지역")) {
                ...
                if (regionVal.isNotEmpty()) {
                    ...
                    // 지역 본인명의 시세/대출/보증금 파싱 → parsedPropertyTotal에 합산
                    else if (lineNoSpace.contains("본인명의") && (lineNoSpace.contains("시세") || lineNoSpace.contains("공시지가") || lineNoSpace.contains("실거래") || lineNoSpace.contains("보증금"))) {
                        ...
                        for (group in groups) {
                            ...
                            // 그룹 내에 "배우자명의 시세/공시지가/실거래/보증금" 있으면 → 배우자 단독 부동산 (본인명의는 대출만)
                            // 시세는 배우자 소유, 본인명의 대출도 배우자 부동산에 묶인 것으로 간주 → ÷2
                            val spouseOwnsProperty = gNs.contains("배우자명의") &&
                                (gNs.contains("배우자명의시세") || gNs.contains("배우자명의공시지가") || gNs.contains("배우자명의실거래") || gNs.contains("배우자명의보증금"))
                            val rawSise = extractAmountAfterKeyword(g, "시세").takeIf { it > 0 }
                                ?: extractAmountAfterKeyword(g, "공시지가").takeIf { it > 0 }
                                ?: extractAmountAfterKeyword(g, "실거래").takeIf { it > 0 }
                ...
                if (isNextField) {
                    inRegionField = false
                } else if (lineNoSpace.contains("시세") || lineNoSpace.contains("공시지가") || lineNoSpace.contains("실거래") || lineNoSpace.contains("보증금") || lineNoSpace.contains("전세")) {
            ...
            if (inPropertySection && !lineNoSpace.contains("채무") && !lineNoSpace.contains("대출과목")) {
                ...
                if (propertyNoSpace.isNotEmpty() && !propertyNoSpace.matches(Regex("[xX×없0].*"))) {
                    ...
                    for (group in groups) {
                        ...
                        val hasSpouseOwnership = !isGroupJoint && gNoSpace.contains("배우자명의") && (gNoSpace.contains("시세") || gNoSpace.contains("빌라") || gNoSpace.contains("아파트") || gNoSpace.contains("건물") || gNoSpace.contains("공시지가") || gNoSpace.contains("실거래") || gNoSpace.contains("보증금"))
                        ...
                        val appliedNet = if (explicitJibunAmt > 0) {
                            ...
                            val net = maxOf(siseAmt - loanAmt - seipjaAmt - deductBojung, 0)
                            when {
                                isSpouseOwned -> 0  // 배우자명의 → 본인 재산 미포함
                                isOthers -> net / 2
                                else -> net
                            }
                        }
                        ...
                        if (isSpouseOwned) {
                            Log.d("HWP_PARSE", "재산 배우자명의 (시세${siseAmt}만 - 대출${loanAmt}만): 재산 미포함")
                        }
                        if (isOthers && !isSpouseOwned) parsedOthersProperty += appliedNet
                    }
            ...
            val isRealEstateLine = (inRegionField || inPropertySection) &&
                    (lineNoSpace.contains("시세") || lineNoSpace.contains("공시지가") || lineNoSpace.contains("실거래") ||
            ...
            if (isRealEstateLine) {
                ...
                if ((lineNoSpace.contains("명의") &&
                    ...
                    hasOthersRealEstate = true
                    val hasSise = lineNoSpace.contains("시세") || lineNoSpace.contains("공시지가") || lineNoSpace.contains("실거래")
            ...
            if ((lineNoSpace.contains("월소득") || lineNoSpace.contains("연봉") || lineNoSpace.contains("순수익")) && !lineNoSpace.contains("배우자") && (lineNoSpace.contains("순수익") || (!lineNoSpace.contains("예상") && !lineNoSpace.contains("예정"))) && !inSpecialNotesSection && !lineNoSpace.contains("특이사항")) {
                if (lineNoSpace.contains("수령") && (lineNoSpace.contains("없") || lineNoSpace.contains("수령0") || lineNoSpace.contains("수령x") || lineNoSpace.contains("수령X"))) {
                    ...
                    if (netIncomeDeficit) {
                        ...
                        // 수당 감지 → 월소득에 가산 (예: "월 소득 237만 + 수당 65만")
                        val sudangMan = Regex("수당\\s*(\\d+)\\s*만").find(line)?.groupValues?.get(1)?.toIntOrNull() ?: 0
                        ...
                        // 수당은 월소득 파싱에서 빠지므로 별도 가산 (월 소득 N만 + 수당 M만)
                        if (sudangMan > 0) {
                            parsedMonthlyIncome += sudangMan
                            parsedSudangTotal += sudangMan
                            Log.d("HWP_PARSE", "수당 소득 합산: +${sudangMan}만 → 합계=${parsedMonthlyIncome}만 ($line)")
                        }
                    }
                    ...
                    if (pensionMatch != null) {
                        ...
                        parsedPensionTotal += pensionAmount
            ...
            if (inSpecialNotesSection) {
                ...
                if (content.isNotEmpty() && !content.equals("x", true)) {
                    ...
                    // 아르바이트/알바 → 소득에 합산 (예: "아르바이트 - 월 100만 3.3%", "알바 80만")
                    val partTimeMatch = Regex("(?:아르바이트|알바)\\s*[-=:：]?\\s*(?:월\\s*)?(\\d+)\\s*만").find(content)
                    if (partTimeMatch != null) {
                        val partTime = partTimeMatch.groupValues[1].toInt()
                        parsedMonthlyIncome += partTime
                        Log.d("HWP_PARSE", "특이사항 아르바이트 → 소득 합산: +${partTime}만 → ${parsedMonthlyIncome}만 ($content)")
                    }
                    // 사업장 보증금 → 재산 합산
            ...
            if (isCarLine) {
                ...
                if (isSpouseCar) {
                    // 배우자 단독 명의 차량은 본인 재산에 포함하지 않음
                    Log.d("HWP_PARSE", "배우자명의 차량 제외: 시세=${carSise}만 (재산 미포함) - $line")
                    return
                }
                // 외제차 여부 판별
            ...
            if (loanYear == 0 && inDebtSection && !inOtherDebtSection && !inLoanCategorySection && !inPropertySection && !inRegionField && lineNoSpace.matches(Regex("^순?번?\\d{1,2}.+\\d+만.*"))) {
                ...
                if (seqCreditorM.find()) {
                    ...
                    if (isCashServiceSeq) {
                        ...
                        // 담보 채무도 신청일자 이후 발생이면 특이 표기용으로 추적 (대상채무엔 미합산)
                        val damboDateCol = if (columns.size >= 4) columns[3] else ""
                        val damboDateM = Regex("(\\d{2})\\.(\\d{2})").find(damboDateCol)
                        if (damboDateM != null && pdfApplicationDate.isNotEmpty() && credName.length >= 2) {
                            val dy = 2000 + damboDateM.groupValues[1].toInt()
                            val dm = damboDateM.groupValues[2].toInt()
                            val ap = pdfApplicationDate.split(".")
                            val ay = ap.getOrNull(0)?.toIntOrNull() ?: 0
                            val am = ap.getOrNull(1)?.toIntOrNull() ?: 0
                            if (ay > 0 && (dy > ay || (dy == ay && dm > am))) {
                                postApplicationDamboCreditors[credName] = (postApplicationDamboCreditors[credName] ?: 0) + amountMan
                                Log.d("HWP_PARSE", "채무현황 순번 신청이후(담보): ${dy}.${dm} > ${ay}.${am} → ${amountMan}만 ($credName)")
                            }
                        }
                        ...
                        // 발생일자 컬럼 추출: "25.04" / "26.02" 등
                        val dateCol = if (columns.size >= 4) columns[3] else ""
                        val seqDateM = Regex("(\\d{2})\\.(\\d{2})").find(dateCol)
                        val seqYear = if (seqDateM != null) 2000 + seqDateM.groupValues[1].toInt() else 0
                        val seqMonth = if (seqDateM != null) seqDateM.groupValues[2].toInt() else 0
                        // 신청일자 이후 채무 판단
                        if (pdfApplicationDate.isNotEmpty() && seqYear > 0) {
                        ...
                        // 6개월 이내 채무 수집 (recentDebtRatio 계산용)
                        if (seqYear > 0 && seqMonth > 0) {
                            val seqCal = Calendar.getInstance().apply { set(seqYear, seqMonth - 1, 15) }
                            val sixMonthsAgo = Calendar.getInstance().apply { add(Calendar.MONTH, -6) }
                            if (seqCal.after(sixMonthsAgo)) {
                                recentDebtEntries.add(Pair(seqCal, amountMan * 10))
                                recentCreditorNames.add(credName)
                                Log.d("HWP_PARSE", "채무현황 6개월 수집: ${seqYear}.${seqMonth} ${amountMan}만 ($credName)")
                            } else {
                                olderCreditorNames.add(credName)
                            }
                        }
            ...
            if (loanYear > 0 && inDebtSection && !inSpecialNotesSection && !inPropertySection && !inRegionField) {
                ...
                if (debtAmount > 0) {
                    ...
                    // 채무현황 표(현황순번 담보여부)가 있고 이 순번이 담보 목록에 없으면 신용으로 강제 (보험계약/지급보증/예적금담보 제외 - 그것들은 별도 처리)
                    // 예/적금담보(200)는 채무현황표 표기와 무관하게 항상 담보 — 본인 예적금이 담보로 잡힌 명백한 담보채무
                    val isYejeokgeumDambo = lineNoSpace.contains("예/적금담보") || lineNoSpace.contains("예적금담보") || lineNoSpace.contains("(200)")
                    // 단, 대출과목 표가 빈 표(헤더만 있고 데이터 없음)인 경우 → 채무현황 표의 대출과목 컬럼을 그대로 활용 (강제 신용 X)
                    // 또한, 채권사명이 대출과목 담보목록에 있고 행 자체가 담보 성격(담보 키워드 또는 290 기타담보) → 사전스캔이 순번을 못 잡은 경우(텍스트박스/서식 누락)도 담보로 인정
                    val isDamboByNameAndLoan = matchesLoanCatDambo(rawCreditorName, loanCatDamboCreditorNames) && !matchesLoanCatDambo(rawCreditorName, loanCatCreditCreditorNames) && (lineNoSpace.contains("담보") || is290 || hasDamboKeyword(lineNoSpace))
                    val isCreditByHwangsang = hasHwangsangSeqHeader && rowSeqNum > 0 && (excludedSeqNumbers.isNotEmpty() || loanCatCreditCreditorNames.isNotEmpty()) && !excludedSeqNumbers.contains(rowSeqNum) && !isInsurancePolicyLoan && !isGuaranteeLoan && !isYejeokgeumDambo && !isDamboByNameAndLoan
                    if (isCreditByHwangsang) Log.d("HWP_PARSE", "채무현황 표에 담보 미표기 → 신용 강제 (대상채무 포함): rowSeqNum=$rowSeqNum - $line")
                    val isDamboLoan = !isJilgwonXCreditor && !isJilgwonXOverride && !isCreditByLoanCatName && !isCreditByHwangsang && ((lineFor담보.contains("담보") || is290DamboByCategory || is290DamboByPdf || isDamboByPreScan || isDamboByLoanCatName || isDamboByNameAndLoan || hasDamboKeyword(lineNoSpace) || (isJeonse270 && (!jeonseNoJilgwon || isJilgwonOCreditor)) || isInsurancePolicyLoan) && !isGuaranteeLoan && !lineNoSpace.contains("(240)") && !lineNoSpace.contains("무담보") && !lineNoSpace.contains("마이너스"))
                    ...
                    if (isJiguBojung && loanYear > 0 && prevBusinessAmount != null && debtAmount <= prevBusinessAmount) {
                        ...
                        if (isCashService) {
                            ...
                            if (isDamboLoan) {
                                ...
                                // 담보 채무도 신청일자 이후 발생이면 특이 표기용 추적 (대상채무 미합산)
                                if (pdfApplicationDate.isNotEmpty() && rawCreditorName.length >= 2 && loanYear > 0) {
                                    val apParts = pdfApplicationDate.split(".")
                                    if (apParts.size == 3) {
                                        val appCal = Calendar.getInstance().apply { set(apParts[0].toInt(), apParts[1].toInt() - 1, apParts[2].toInt()) }
                                        val dCal = Calendar.getInstance().apply { set(loanYear, loanMonth - 1, if (loanDay > 0) loanDay else 15) }
                                        if (dCal.after(appCal)) {
                                            postApplicationDamboCreditors[rawCreditorName] = (postApplicationDamboCreditors[rawCreditorName] ?: 0) + (debtAmount + 5) / 10
                                            Log.d("HWP_PARSE", "신청일자 이후 채무(담보): ${loanYear}.${loanMonth}.${loanDay} ${(debtAmount + 5) / 10}만 - $line")
                                        }
                                    }
                                }
                ...
                if (loanCatSeqM.find() || excludeSeqM.find()) {
                    ...
                    if (afterSeq.isNotEmpty()) {
                        ...
                        if (dkM.find()) {
                            ...
                            // "완납" 표시 분리 (협약외 채권사가 이미 변제됨 → PDF 제외보증서담보 차감용)
                            val isPaidOff = afterSeq.contains("완납")
                            val creditorClean = if (isPaidOff) afterSeq.replace("완납", "").trim() else afterSeq
                            loanCatRows.add(LoanCatRow(seqStr, creditorClean, ""))
                            if (isExcludeEntry) Log.d("HWP_PARSE", "대출과목 협약외 감지: $seqStr $creditorClean${if (isPaidOff) " 완납" else ""} - $line")
                            if (isExcludeEntry && isPaidOff) {
                                val seqNum = excludeSeqM.group(1)?.toIntOrNull()
                                if (seqNum != null) {
                                    paidOffExcludeSeqs.add(seqNum)
                                    Log.d("HWP_PARSE", "대출과목 완납 채권사 감지: 제외$seqNum $creditorClean (PDF 제외채무 ${seqNum}번째 행 차감 예정)")
                                }
                            }
                        }
                    }
            ...
            if (lineNoSpace.contains("카드사") && lineNoSpace.contains("이용금액")) {
                ...
                // 병합표 마지막 행 케이스: "농협 311만 480만 기타채무 채권내용 채무금액"처럼
                // 좌측 카드 행과 우측 다음 섹션 헤더가 한 줄로 합쳐질 수 있음
                // → 카드 행 파싱을 먼저 시도하고, 카드 데이터가 있으면 섹션 종료 유보 (다음 카드 행 받기)
                val hasSectionEndKeyword = lineNoSpace.contains("대출과목") || lineNoSpace.contains("기타채무") || lineNoSpace.contains("요약") ||
                    lineNoSpace.contains("특이") || lineNoSpace.contains("채무조정")
                var cardRowParsed = false
                run {
                ...
                // 카드 행이 파싱된 라인은 종료 키워드가 있어도 섹션 유지 (병합표에서 다음 카드 행이 따로 올 수 있음)
                if (hasSectionEndKeyword && !cardRowParsed) inCardUsageTableSection = false
            }
        ...
        // 학자금 포함 여부는 PDF 병합(parsedCreditorMap 덮어쓰기) 전 원본 기준으로 판단
        // (신청전 학자금은 mergedMap=PDF조정+신청이후 에서 빠지므로 병합 후 검사하면 누락됨)
        ...
        // 차량담보 reclassify: 차량 파싱에서 검출된 담보/할부 금액 + 사이드바 담보 채권사명 매칭 → 담보로 이동
        // 금액 매칭으로 동일 채권사의 다른 채무(신용 등) 오분류 방지
        // 차량할부도 포함 (PDF 코드가 신용/지급보증이어도 사이드바가 차량담보로 분류했으면 차량담보 처리)
        for (carDambo in carDamboAmountList + carHalbuAmountList) {
        ...
        // 완납 채권사 제외 (특이사항에서 감지된 brand keyword) - 채무현황 금액만 제외, 카드이용금액은 유지
        ...
        // PDF 제외보증서담보 중 대출과목 "제외N 완납"으로 표시된 행 차감 (인덱스 매칭)
        if (pdfExcludedEntries.isNotEmpty() && pdfExcludedGuaranteeDebt > 0 && paidOffExcludeSeqs.isNotEmpty()) {
            var paidOffBoundary = 0
            val removedBoundary = mutableListOf<String>()
            for (entry in pdfExcludedEntries) {
                if (entry.isDambo) continue  // 진짜 담보(차량/주택/할부)는 대상채무 미포함이므로 스킵
                if (entry.seq in paidOffExcludeSeqs) {
                    paidOffBoundary += entry.principal
                    removedBoundary.add("제외${entry.seq} ${entry.name}(${entry.principal}만)")
                }
            }
            if (paidOffBoundary > 0) {
                pdfExcludedGuaranteeDebt = (pdfExcludedGuaranteeDebt - paidOffBoundary).coerceAtLeast(0)
                Log.d("HWP_CALC", "완납 제외보증서담보 차감: ${removedBoundary.joinToString()} → -${paidOffBoundary}만 → 제외보증서=${pdfExcludedGuaranteeDebt}만")
            }
        }
        ...
        if (pdfAgreementCreditors.isNotEmpty()) {
            ...
            // PDF 합의서 처리 전 한글 양식 채권사 보존 (PDF 제외채무와 매칭 판단용)
            val hwpOriginalCreditorNames = parsedCreditorMap.keys.toSet()
            ...
            // PDF 제외채무와 한글 채무현황을 채권사명+금액으로 비교: 같으면(같은 채무) 한글 우선, 다르면 PDF 사용
            if (pdfAgreementDebt > 0) {
                val hwpTextDebt = totalParsedDebt - parsedDamboTotal
                val pdfExcludedAllSum = pdfExcludedEntries.filter { it.principal > 0 }.sumOf { it.principal }
                val pdfExcludedAllNames = pdfExcludedEntries
                    .filter { it.principal > 0 && it.name.length >= 2 }
                    .map { normalizeCreditorName(it.name) }
                    .toSet()
                val hwpNamesNormalized = hwpOriginalCreditorNames.map { normalizeCreditorName(it) }.toSet()
                val pdfHwpNameMatch = pdfExcludedAllNames.count { pdfName ->
                    hwpNamesNormalized.any { hwpName ->
                        pdfName.length >= 3 && hwpName.length >= 3 &&
                        (pdfName.contains(hwpName.take(3)) || hwpName.contains(pdfName.take(3)))
                    }
                }
                val nameMatchRatio = if (pdfExcludedAllNames.isEmpty()) 0.0
                    else pdfHwpNameMatch.toDouble() / pdfExcludedAllNames.size
                val amountMatchRatio = if (pdfExcludedAllSum > 0 && hwpTextDebt > 0)
                    minOf(pdfExcludedAllSum, hwpTextDebt).toDouble() / maxOf(pdfExcludedAllSum, hwpTextDebt) else 0.0
                val sameDebt = hwpTextDebt > 0 && pdfExcludedAllSum > 0 && nameMatchRatio >= 0.5 && amountMatchRatio >= 0.5
                // 신청이후 채무는 postApplicationDebtMan으로 별도 합산되므로 hwpTextDebt에서 제외 (이중 카운트 방지)
                val hwpTextDebtPreApplication = (hwpTextDebt - postApplicationDebtMan).coerceAtLeast(0)
                val excludedDebt = when {
                    sameDebt -> hwpTextDebt  // 같은 채무 → 한글 우선
                    hwpTextDebtPreApplication > 0 -> pdfExcludedGuaranteeDebt + hwpTextDebtPreApplication  // 다른 채무 → 둘 다 더함 (신청이후 제외)
                    else -> pdfExcludedGuaranteeDebt  // 한글 없음 → PDF만
                }
                val excludedLabel = when {
                    sameDebt -> "한글채무현황"
                    hwpTextDebt > 0 -> "제외보증서+한글채무현황"
                    else -> "제외보증서"
                }
                targetDebt = pdfAgreementDebt + excludedDebt + postApplicationDebtMan + parsedCardUsageTotal + telecomDebt
                Log.d("HWP_PARSE", "합의서+한글 대상채무 합산: 합의서=${pdfAgreementDebt}만 + ${excludedLabel}=${excludedDebt}만 + 신청이후=${postApplicationDebtMan}만 + 카드=${parsedCardUsageTotal}만 + 통신=${telecomDebt}만 = ${targetDebt}만 (이름매칭=${(nameMatchRatio * 100).toInt()}%, 금액매칭=${(amountMatchRatio * 100).toInt()}%, 같은채무=$sameDebt)")
            }
        ...
        val financialKeywords = listOf("은행", "카드", "캐피탈", "저축", "대부", "보험", "증권", "금융", "신협", "농협", "수협", "새마을", "조합", "재단", "보증", "자산관리", "추심", "생명", "화재", "손해", "상사", "할부", "여신", "리스", "펀드", "투자", "파이낸스", "캐시", "론", "크레디트", "신용정보", "상호", "공사", "에스비아이", "웰컴", "오케이", "페퍼", "제이티", "모아", "예스", "에이앤피", "테크놀로지", "핀테크")
        ...
        // 단기(회생): 배우자 모르게이면 미성년 자녀 절반만 인정 (자녀 1명은 그대로 1명, 2명 이상부터 절반 floor)
        // 기혼+배우자 소득 있으면 미성년 절반 (자녀 1명은 그대로 1명, 2명 이상부터 절반 ceiling)
        val effectiveMinorForHoeseng = when {
            spouseSecret -> if (minorChildren <= 1) minorChildren else minorChildren / 2
            hasSpouse && spouseHasIncome -> if (minorChildren <= 1) minorChildren else (minorChildren + 1) / 2
            else -> minorChildren
        }
        ...
        // (parsedIncludesStudentLoan은 PDF 병합 전 원본 기준으로 위에서 이미 판단)
...
// 남양주 과밀억제권역 해당 지역 (이것만 포함)
        ...
        // 상환내역서(신복위 진행) 신청일 이후 발생 채무 → 특이 표기 (담보 포함, 대상채무엔 별도 합산)
        if (postApplicationCreditors.isNotEmpty() || postApplicationDamboCreditors.isNotEmpty()) {
            val postAppCombined = LinkedHashMap<String, Int>()
            for ((n, a) in postApplicationCreditors) postAppCombined[n] = (postAppCombined[n] ?: 0) + a
            for ((n, a) in postApplicationDamboCreditors) postAppCombined[n] = (postAppCombined[n] ?: 0) + a
            val postAppTotal = postAppCombined.values.sum()
            specialNotesList.add("상환내역서 이후 채무 ${formatToEok(postAppTotal)} (${postAppCombined.keys.joinToString(",")})")
        }
        ...
        // 1인가구 + 소득 250만 이상 → 장기 기간 1년 단축 후 월 변제금 재계산
        // 단, 부모0(60세이상 부모 없음)이면 이미 가용소득 2/3로 완화됐으므로 -1년 미적용 (중복 완화 방지)
        if (!longTermFullyBlocked && finalYear > 2 && income >= 250 && householdForShinbok == 1 && parentCount > 0 && !(isFreelancer || noSocialInsurance)) {
            ...
            var candMonthly = if (newMonths > 0) {
                if (longTermIsFullPayment) maxOf(totalPayment / newMonths, 40)  // 원금전액: 1만 단위 내림 (대상채무 초과 방지), 40만 하한
                else (totalPayment + newMonths - 1) / newMonths
            } else oldMonthly
            if (!longTermIsFullPayment) candMonthly = candMonthly / 5 * 5  // 5만 단위 내림 (대상채무 초과 방지)
            // 내림 후 40만보다 작으면 년수를 더 줄임
            while (candYear > 1 && candMonthly < 40) {
                candYear -= 1
                newMonths = candYear * 12
                candMonthly = if (newMonths > 0) {
                    if (longTermIsFullPayment) maxOf(totalPayment / newMonths, 40)
                    else (totalPayment + newMonths - 1) / newMonths
                } else candMonthly
                if (!longTermIsFullPayment) candMonthly = candMonthly / 5 * 5
            }
            // 단축한 월변제금이 가용소득을 초과하면 -1년 미적용 (실현 불가능한 변제금 방지)
            val availableForLong = availableIncome - parentDeduction
            if (availableForLong > 0 && candMonthly > availableForLong) {
                Log.d("HWP_CALC", "1인가구+소득250↑ -1년 미적용: ${candYear}년 월변제금 ${candMonthly}만 > 가용소득 ${availableForLong}만 → ${oldYear}년/${oldMonthly}만 유지")
            } else {
                finalYear = candYear
        ...
        if (diagnosis == "단순유리" && !longTermFullyBlocked && !nonAffiliatedOver20 && roundedAggressiveMonthly > 0 && aggressiveYears > 0) {
        ...
        if (recentDebtRatio >= 30 && recentDebtEntries.isNotEmpty() && targetDebt > 0 && !skipRecentDateCalc && (isSaeDiagnosis || !(effectiveShortTermBlocked && longTermFullyBlocked))) {
            ...
            for ((date, amountChon) in recentDebtEntries) {
                ...
                if (newRatio < 30) {
                    ...
                    // 면책 해소 임박이라도 회생 불가 조건(배우자모르게/재산초과) 있으면 "회" 추가 X → "워" 유지
                    // 면책 외 다른 회생 차단 사유(채무한도/한국주택금융/소득100이하)가 있으면 면책 해소돼도 회생 불가 → 회 미추가
                val dischargeEndSoonAllowsHoe = dischargeEndSoon && !spouseSecret && !longTermPropertyExcess && !shortTermDebtOverLimit && !hasHfcMortgage && !lowIncome
                    val afterDiag = if (isSaeDiagnosis) {
                        diagnosis  // 새출발 진단명 유지 (새새/회새/새)
                    } else if (hoeBlocked || effectiveShortTermBlocked) when {
                        delinquentDays >= 90 -> if (dischargeEndSoonAllowsHoe) (if (longBetter) "워유워" else "워유회") else if (isDischargeBanned) "워유워" else "회워"
                        delinquentDays >= 30 -> if (dischargeEndSoonAllowsHoe) (if (longBetter) "프유워" else "프유회") else "프유워"
                        else -> if (dischargeEndSoonAllowsHoe) (if (longBetter) "신유워" else "신유회") else "신유워"
                    } else when {
                        delinquentDays >= 90 -> if (dischargeEndSoonAllowsHoe) (if (longBetter) "워유워" else "워유회") else "회워"
                        delinquentDays >= 30 -> if (dischargeEndSoonAllowsHoe) (if (longBetter) "프유워" else "프유회") else "프회워"
                        else -> if (dischargeEndSoonAllowsHoe) (if (longBetter) "신유워" else "신유회") else "신회워"
                    }
        ...
        // [소득] 분해 표시: 기본급여 + 양육비(±) + 연금 = 합계
        val hasPensionBreakdown = parsedPensionTotal > 0
        val hasChildSupportBreakdown = childSupportAmount > 0
        val hasSudangBreakdown = parsedSudangTotal > 0
        binding.card.text = if (hasPensionBreakdown || hasChildSupportBreakdown || hasSudangBreakdown) {
            val baseSalary = parsedMonthlyIncome - parsedPensionTotal - parsedSudangTotal
            val csSigned = if (!hasChildSupportBreakdown) 0 else if (childSupportReceiving) childSupportAmount else -childSupportAmount
            val parts = mutableListOf<String>()
            parts.add(baseSalary.toString())
            if (hasChildSupportBreakdown) parts.add("양육비 $csSigned")
            if (hasSudangBreakdown) parts.add("수당 $parsedSudangTotal")
            if (hasPensionBreakdown) parts.add("연금 $parsedPensionTotal")
            val totalDisplay = baseSalary + csSigned + parsedSudangTotal + parsedPensionTotal
            "[소득] ${parts.joinToString(" + ")} = ${totalDisplay}만${incomeSuffix}"
        } else {
            "[소득] ${dispIncome}만${incomeSuffix}"
        }
        ...
        val longTermDiagLabel = if (longTermFullyBlocked || longTermDebtOverLimit) "" else {
            var label = if (diagnosis.startsWith("(") && diagnosis.endsWith("회")) {
                ...
                // 회생 가능 → 회 path (6개월 30%↑여도 회생은 비율 제약 없음; 회→유 변환은 5847~5853에서 별도 처리)
                when {
                    delinquentDays >= 90 -> "회워"
                    delinquentDays >= 30 -> "프회워"
                    else -> "신회워"
            ...
            if (canDeferment) {
                ...
                // 소액(4000만 이하)은 단기 불가일 때만 회→유 (단기 가능하면 회생 추천 유지)
                if (isBusinessOwner || shortTermPropertyExcess || shortTermHomeAuctionRisk || hasSameCreditorDamboAndCredit || hasHfcMortgage || (targetDebt <= 4000 && effectiveShortTermBlocked)) {
        ...
        // 회로 끝나는 라벨(회생 기준)은 최대 6년 → 6년 초과 표기 금지
        val overrideYear = if (labelEndsWithHoe) {
            val base = if (shortTermMonths > 0) maxOf(1, (shortTermMonths + 3) / 12) + 1 else finalYear
            minOf(6, maxOf(1, base))
        } else 0
        // 장기 불가 사유 수집
        ...
        // 중간 회→유 변환: 사업자, 단순재산초과, 집경매 위험, 한국주택금융공사 집담보, 동일 채권사가 담보+신용 보유, 또는 대상채무 4000만 이하
        val hasSameCreditorDamboAndCredit = parsedDamboCreditorNames.any { normalizeCreditorName(it) in normalizedCreditorMap }
        if (canDeferment && (isBusinessOwner || shortTermPropertyExcess || shortTermHomeAuctionRisk || hasSameCreditorDamboAndCredit || hasHfcMortgage || (targetDebt <= 4000 && effectiveShortTermBlocked))) {
        ...
        if (longTermBlockedAny && !isBangsaeng) {
            ...
            diagnosis = when {
                ...
                // 회생 불가 조건이면 단기(회/단순유리) 카테고리 자체가 불가 → 장기까지 막히면 방생 (새출발은 별도 분기에서 처리)
                diagnosis in standardLabels && hoeBlocked -> "방생"
                diagnosis in standardLabels -> if (effectiveShortTermBlocked) "방생" else "단순유리"  // 회생 가능 + 단기 가능시 단순유리, 불가시 방생
                else -> diagnosis
            }
            if (before != diagnosis) Log.d("HWP_CALC", "장기 불가 → 진단 동기화: $before → $diagnosis (한도초과=$longTermDebtOverLimit, 미협약=$nonAffiliatedOver20, 장기차단=$longTermFullyBlocked)")
        }
        ...
        // 단기 회생 가능(회생 가능 & 단기 가능)인데 라벨은 회워, 코어가 유워면 동기화 금지 → 장기 라벨의 회 보존
        val hoeToYuDowngrade = longTermDiagLabel.endsWith("회워") && diagnosisCore.endsWith("유워") && !hoeBlocked && !effectiveShortTermBlocked
        var effLtLabel = longTermDiagLabel
        if (!hoeToYuDowngrade && diagnosisCore in ltCoreLabels && longTermDiagLabel.isNotEmpty() && longTermDiagLabel != diagnosisCore) {
        ...
        // 진행중 제도 prefix를 장기 라벨에도 반영 (유예기간 있으면 진단과 동일하게 유 추가)
        if (hasOngoingProcess && ongoingProcessName.isNotEmpty()) {
            val ltText = longTermText.toString()
            val ongoingPrefix = "($ongoingProcessName)"
            val ongoingPrefixYu = "($ongoingProcessName)유"
            // " / 회워" → " / (신)유회워" (유예 1~11개월) 또는 " / (신유)회워" (유예 12개월 이상) 또는 " / (신)회워" (유예 없음)
            for (diag in listOf("회워", "신유워", "프유워", "신회워", "프회워", "워유워", "워회워")) {
                if (ltText.contains(" / $diag") && !ltText.contains(" / $ongoingPrefix") && !ltText.contains(" / $ongoingPrefixYu")) {
                    // baseDiag: 연체 prefix 제거(신/프/워) → 회생 prefix 중복 제거 → 유 중복 제거
                    var baseDiag = diag
                    if (baseDiag.startsWith("신") || baseDiag.startsWith("프") || baseDiag.startsWith("워")) {
                        baseDiag = baseDiag.substring(1)
                    }
                    if (ongoingProcessName == "회" && baseDiag.startsWith("회") && !baseDiag.startsWith("회새")) {
                        baseDiag = baseDiag.substring(1)
                    }
                    if (baseDiag.startsWith("워유워")) baseDiag = baseDiag.removePrefix("워유")
                    else if (baseDiag.startsWith("유")) baseDiag = baseDiag.removePrefix("유")
        ...
        binding.test2.text = longTermText.toString()
        // 최종 단순유리 + 6개월 30% 초과 → 비율 해소(x월) 이후 장기(신복위 워크아웃) 가능 → 진단에 "x월 이후 회워 가능" 추가
        if (diagnosis == "단순유리" && recentDebtRatio >= 30 && recentDebtEntries.isNotEmpty() && targetDebt > 0 && longTermDiagLabel.isNotEmpty()) {
            findThresholdDate(recentDebtEntries, recentDebtMan, totalDebtForRatio)?.let { pd ->
                finalDiagnosis = "$finalDiagnosis, $pd 이후 회워 가능"
                Log.d("HWP_CALC", "단순유리 최종 + 6개월30% → 진단 $pd 이후 회워 가능")
            }
        }
    ...
    private fun calcLongTermValues(
        ...
        // 미혼·부양가족없음·60세이상부모없음(1인가구+부모0) → 장기 변제금은 가용소득의 2/3로 완화
        if (householdForShinbok == 1 && parentCount == 0 && monthly > minMonthly) {
            monthly = maxOf(minMonthly, monthly * 2 / 3)
        }
        ...
        if (conservativeMonthly > 0) {
            ...
            conservativeYears = conservativeYears.coerceIn(2, maxYears)
            val calcMaxYears = Math.round(totalPayment.toDouble() / (conservativeMonthly * 12)).toInt().coerceAtLeast(2)
        ...
        if (conservativeYears > 0 && conservativeMonthly * conservativeYears * 12 > targetDebt) {
            val reducedYears = maxOf(2, totalPayment / (conservativeMonthly * 12))
            if (conservativeMonthly * reducedYears * 12 <= totalPayment) {
                ...
                if (adjustedMonthly < minMonthly && conservativeYears > 1) {
                    conservativeMonthly = minMonthly
                    conservativeYears = Math.round(totalPayment.toDouble() / minMonthly / 12.0).toInt().coerceAtLeast(2)
                } else {
        ...
        if (conservativeYears < 2) conservativeYears = 2
        ...
        if (finalMonthly < minMonthly) {
            ...
            val calcMaxYears = Math.round(totalPayment.toDouble() / (minMonthly * 12)).toInt().coerceAtLeast(2)
        ...
        if (checkMonths > 0 && finalMonthly * checkMonths > targetDebt) {
            ...
            if (newMonthly >= minMonthly) {
                ...
                finalYear = maxOf(2, targetDebt / (minMonthly * 12))
                useMonths = false
                displayMonths = 0
        ...
        // ★ 소득 300만+ 자녀 없음(1인가구) + 60세이상 부모: 월변제금 2/3 감액 후 연수 재계산
        if (income >= 300 && householdForShinbok == 1 && parentCount > 0 && finalMonthly > 0 && totalPayment > 0) {
            val origMonthly = finalMonthly
            val origYear = finalYear
            var newMonthly = Math.ceil(finalMonthly * 2.0 / 3.0).toInt().coerceAtLeast(minMonthly)
            if (!isFullPayment) newMonthly = (newMonthly + 2) / 5 * 5
            var newMonths = Math.ceil(totalPayment.toDouble() / newMonthly).toInt()
            if (newMonths > maxMonths) newMonths = maxMonths
            var newYear = ((newMonths + 11) / 12).coerceIn(2, maxYears)
            // 년수 cap된 경우 월변제금 재계산
            if (newMonthly * newYear * 12 < totalPayment) {
                newMonthly = Math.ceil(totalPayment.toDouble() / (newYear * 12)).toInt()
                if (!isFullPayment) newMonthly = (newMonthly + 2) / 5 * 5
            }
            finalMonthly = newMonthly
            finalYear = newYear
            useMonths = false
            displayMonths = 0
            Log.d("HWP_CALC", "소득300+자녀0+60세부모: 월변제금 ${origMonthly}만→${finalMonthly}만(2/3), 연수 ${origYear}→${finalYear}년")
        }
    ...
    private fun calculateClientDiagnosis(
        ...
        // 1인가구 + 소득 250만 이상 → 장기 기간 1년 단축 후 월 변제금 재계산
        // 단, 부모0(60세이상 부모 없음)이면 이미 가용소득 2/3로 완화됐으므로 -1년 미적용 (중복 완화 방지)
        if (clientFinalYear > 2 && income >= 250 && householdForShinbok == 1 && parentCount > 0 && !isFreelancer) {
            val oldYear = clientFinalYear; val oldMonthly = clientFinalMonthly
            var candYear = clientFinalYear - 1
            var newMonths = candYear * 12
            var candMonthly = if (newMonths > 0) {
                if (longTermIsFullPayment) maxOf(totalPayment / newMonths, 50)  // 원금전액: 1만 단위 내림 (대상채무 초과 방지), 50만 하한
                else (totalPayment + newMonths - 1) / newMonths
            } else oldMonthly
            if (!longTermIsFullPayment) candMonthly = candMonthly / 5 * 5  // 5만 단위 내림 (대상채무 초과 방지)
            // 내림 후 50만보다 작으면 년수를 더 줄임
            while (candYear > 1 && candMonthly < 50) {
                candYear -= 1
                newMonths = candYear * 12
                candMonthly = if (newMonths > 0) {
                    if (longTermIsFullPayment) maxOf(totalPayment / newMonths, 50)
                    else (totalPayment + newMonths - 1) / newMonths
                } else candMonthly
                if (!longTermIsFullPayment) candMonthly = candMonthly / 5 * 5
            }
            // 단축한 월변제금이 가용소득을 초과하면 -1년 미적용 (본체와 동일)
            val availableForLong = income - livingCostShinbok - parentDeduction
            if (availableForLong > 0 && candMonthly > availableForLong) {
                Log.d("HWP_CALC", "거래처 1인가구+소득250↑ -1년 미적용: ${candYear}년 월변제금 ${candMonthly}만 > 가용소득 ${availableForLong}만 → ${oldYear}년/${oldMonthly}만 유지")
            } else {
                clientFinalYear = candYear
                clientFinalMonthly = candMonthly
        ...
        if (isClientMode) {
            ...
            if (legoHasResult && clientFinalMonthly > 0) {
                ...
                // 회로 끝나는 라벨(회생 기준)은 최대 6년 → 6년 초과 표기 금지 (본체와 동일)
                val clientHoeYear = if (clientLabelEndsWithHoe) minOf(6, maxOf(1, if (shortTermMonthsForClient > 0) (shortTermMonthsForClient + 3) / 12 + 1 else clientFinalYear)) else 0
                val clientPeriodStr = if (clientHoeYear > 0) "${clientHoeYear}년납"
    ...
    // 공동명의 본인 지분율(%) 파싱. "본인지분 33%" → 33, "본인4:어머니4:누나2" → 40%, "본인 50" → 50%
    // 본인 숫자 뒤에 콜론이 오면 비율 표기로 보고 분모 합산하여 %로 변환
    private fun parseOwnerRatioPct(text: String): Int? {
        // "본인지분 33%" 또는 "본인 지분 33%" 형식 우선 처리
        val jibunPctMatch = Regex("본인\\s*지분\\s*(\\d+)\\s*%").find(text)
        if (jibunPctMatch != null) return jibunPctMatch.groupValues[1].toIntOrNull()
