...
[1] 대상채무 총액 (원 단위)
- "채권자별 변제예정액의 산정내역" 테이블에서 "합계" 행의 "(D)개인회생채권액" 확정채권액
- 또는 "개인회생채권 목록" 등의 테이블에서 "합계"/"총계" 행의 가장 큰 금액

[2] 총변제예정액 (원 단위)
- "채권자별 변제예정액의 산정내역" 테이블에서 "합계" 행의 "(F)총변제예정(유보)액" 확정채권액
- 가용소득이 아닌 총변제예정액을 추출하세요
...
반드시 JSON만 응답:
{"totalDebt": 원단위숫자, "totalRepayment": 총변제예정액원단위숫자, "repaymentMonths": 개월수숫자}"""
        ...
        Log.d("FILE_PROCESS", "변제계획안 Claude 결과: 대상채무=${pdfRecoveryDebt}만, 총변제예정액=${totalRepayment / 10000}만, 월변제금=${pdfRecoveryIncome}만, 변제기간=${pdfRecoveryMonths}개월 ($fileName)")
    }
    ...
    private fun parseHwpData(text: String) {
        ...
        for (rawLine in lines) {
            ...
            if (fieldCheckStr.startsWith("지역")) {
                ...
                if (regionVal.isNotEmpty()) {
                    ...
                    // 지역 본인명의 시세/대출/보증금 파싱 → parsedPropertyTotal에 합산
                        ...
                        if (!jeonseNoJilgwon && (lineNoSpace.contains("질권설정x") || lineNoSpace.contains("질권설정X") || lineNoSpace.contains("질권x") || lineNoSpace.contains("질권X") || lineNoSpace.contains("질권설정안") || line.contains("질권설정 x") || line.contains("질권설정 X") || line.contains("질권 x") || line.contains("질권 X") || line.contains("질권설정 안"))) {
                        ...
                        for (group in groups) {
                            ...
                            val net = if (isJilgwonXProperty) siseAmt else maxOf(siseAmt - loanAmt - deductBojung, 0)
            ...
            if ((lineNoSpace.contains("월소득") || lineNoSpace.contains("연봉") || lineNoSpace.contains("순수익")) && !lineNoSpace.contains("배우자") && !lineNoSpace.contains("예상") && !lineNoSpace.contains("예정") && !inSpecialNotesSection) {
                if (lineNoSpace.contains("수령") && (lineNoSpace.contains("없") || lineNoSpace.contains("수령0") || lineNoSpace.contains("수령x") || lineNoSpace.contains("수령X"))) {
                    ...
                    if (netIncomeDeficit) {
                        ...
                        if (netMax > 0) {
                            if (lineMonthlyIncome > 0 && lineMonthlyIncome > netMax) {
                                // 월소득 > 순수익 → 월소득 사용
                                parsedMonthlyIncome += lineMonthlyIncome
                                Log.d("HWP_PARSE", "월소득 vs 순수익: 월소득${lineMonthlyIncome}만 > 순수익${netMax}만 → 월소득 사용 ($line)")
                            } else {
                                // 순수익 >= 월소득 → 순수익 사용 + 사업자 표시
                                parsedMonthlyIncome += netMax
                                if (lineMonthlyIncome > 0) {
                                    // 순수익이 더 크면 사업자 + 프리랜서 진단 (특이사항에 "프리랜서"는 안 적음)
                                    if (!specialNotesList.any { it.contains("사업자") }) specialNotesList.add("사업자")
                                    isFreelancer = true
                                    isFreelancerByNetIncome = true  // 순수익 비교로 인한 프리랜서 (특이사항 표시 제외)
                                    Log.d("HWP_PARSE", "월소득 vs 순수익: 순수익${netMax}만 >= 월소득${lineMonthlyIncome}만 → 순수익 사용, 사업자/프리랜서 설정 ($line)")
                                } else {
                            ...
                            if (pensionMatch != null) {
                                ...
                                Log.d("HWP_PARSE", "연금 추가: ${pensionAmount}만 → 합계=${parsedMonthlyIncome}만")
                            }
                        }
                    } else {
            ...
            if (lineNoSpace.contains("결혼여부") || lineNoSpace.contains("결혼")) {
                ...
                if (lineNoSpace.contains("양육비")) {
                    ...
                    // 입금받음/입금중 → 수급 (본인 양육), "주는거 없음" 등은 지급이 아닌 수급
                    if (lineNoSpace.contains("입금받") || lineNoSpace.contains("입금중") || lineNoSpace.contains("주는거없")) {
                        childSupportReceiving = true
                    }
            ...
            if (lineNoSpace.contains("채무조정")) {
                ...
                if (!content.equals("x", true) && content.isNotEmpty()) {
                    ...
                    val contentHasAnyResult = content.contains("면책") || content.contains("면채") || content.contains("실효") ||
                    ...
                    for (seg in debtAdjSegments) {
                        ...
                        val segHasResult = seg.contains("면책") || seg.contains("면채") || seg.contains("실효") ||
                                seg.contains("폐지") || seg.contains("기각") || seg.contains("취하") || seg.contains("완납") ||
                                seg.contains("안되") || seg.contains("안됬") || seg.contains("거절") || seg.contains("반려")
                        if (!segHasResult && !contentHasAnyResult) {
                            if (segNoSpace == "회생" || segNoSpace == "워크아웃" || segNoSpace == "신복위" || segNoSpace == "신속") {
                                // 제도명만 단독 → 무시
                            } else {
            ...
            if (isCarLine) {
                ...
                if (carPaymentM.find()) carMonthly = carPaymentM.group(1)!!.toInt()
                ...
                if (isSpouseCar) {
                    // 배우자 단독 명의 차량은 재산에 포함하지 않음 (공동명의는 ÷2로 포함)
            ...
            if (lineNoSpace.contains("카드사") && lineNoSpace.contains("이용금액")) {
                ...
                if (lineNoSpace.contains("대출과목") || lineNoSpace.contains("기타채무") || lineNoSpace.contains("요약") ||
                    ...
                    // 카드명 뒤에 x/X가 이용금액 자리에 있으면 0만 처리 (x 뒤 금액은 이용한도이므로 무시)
                    ...
                    if (!hasXBeforeAmount && cardAmountMatcher.find()) {
                        ...
                        // 이용금액이 x인 경우 → 0만 처리, 카드사만 등록 (x 뒤 이용한도 금액도 제거)
        ...
        // 100%: 지급보증+대부 합계 70% 이상
        else if (guaranteeDaebuRatio >= 70) {
            repaymentRate = 100; rateReason = if (daebuDebtMan > guaranteeDebtMan * 1.5) "대부 과반"
                else if (guaranteeDebtMan > daebuDebtMan * 1.5) "지급보증 과반"
                else "지급보증/대부 과반"
        }
        ...
        var shortTermHardBlocked = (dischargeWithin5Years && !dischargeEndsSameYear) || (originalNetProperty > shortTermDebt && shortTermDebt > 0)
        if (shortTermBlocked && shortTermHardBlocked) {
            ...
            if (originalNetProperty <= 0) {
                ...
                if (propertyMonths <= 36) {
                    ...
                    shortTermMonthly = Math.ceil(originalNetProperty.toDouble() / 60).toInt()
                    shortTermMonths = 60
                }
            }
            shortTermResult = "${shortTermMonthly}만 / ${shortTermMonths}개월납"
            Log.d("HWP_CALC", "단기 계산: 가용소득=${shortTermMonthly}만, 재산=${originalNetProperty}만, ${shortTermMonths}개월")
        }
...
// 남양주 과밀억제권역 해당 지역 (이것만 포함)
        ...
        if (rentalDeposit > 0 && rentalDeposit <= baseExemption) {
            ...
            val deductedProperty = maxOf(originalNetProperty - rentalDeduction, 0)
            ...
            if (shortTermBlocked && shortTermBlockReason == "재산초과" && (deductedProperty <= shortTermDebt || shortTermDebt <= 0)) {
                ...
                // 단기 결과 재계산
                if (!shortTermHardBlocked && income > 100 && shortTermMonthly > 0) {
                    if (originalNetProperty <= 0) {
                        shortTermMonths = 36
                    } else {
                        val propertyMonths = Math.ceil(originalNetProperty.toDouble() / shortTermMonthly).toInt()
        ...
        val targetDebtExGuarantee = if (delinquentDays < 90) maxOf(targetDebt - guaranteeDebtMan, 0) else targetDebt
        var longTermPropertyExcess = netPropertyAfterExemption > targetDebtExGuarantee && targetDebtExGuarantee > 0
        Log.d("HWP_CALC", "장기 재산초과 판단: ($netProperty - $exemptionAmount) = $netPropertyAfterExemption > ${targetDebtExGuarantee}(대상${targetDebt}${if (delinquentDays < 90) "-지급보증${guaranteeDebtMan}" else ""}) → $longTermPropertyExcess")
        ...
        if (needsCarDisposal && !longTermPropertyExcess) {
            ...
            val dispNames = disposeCars.map { if (it < carNameList.size) carNameList[it] else "차량${it + 1}" }
            fun carDispStr(idx: Int): String {
                val info = carInfoList[idx]
                val name = if (idx < carNameList.size) carNameList[idx] else "차량${idx + 1}"
                return "$name (시세${info[0]}만 / 담보${info[1]}만 / 월납부${info[2]}만)"
            }
            if (carCount == 1 && disposeCars.size == 1) {
                val idx = disposeCars[0]
                val name = if (idx < carNameList.size) carNameList[idx] else "차량1"
                val info = carInfoList[idx]
                val label = if (info[2] in 50..55) "일부 상환 필요" else "처분필요"
                specialNotesList.add("$name $label (시세${info[0]}만 / 담보${info[1]}만 / 월납부${info[2]}만)")
            } else if (carCount >= 2) {
            ...
            val carName = if (carNameList.isNotEmpty()) carNameList[0] else "차량"
            val label = if (carMonthlyPayment in 50..55) "일부 상환 필요" else "처분필요"
            specialNotesList.add("$carName $label (시세${carTotalSise}만 / 담보${carTotalLoan}만 / 월납부${carMonthlyPayment}만)")
        ...
        if (guaranteeDaebuRatio >= 70) specialNotesList.add(
            if (daebuDebtMan > guaranteeDebtMan * 1.5) "대부 과반"
            else if (guaranteeDebtMan > daebuDebtMan * 1.5) "지급보증 과반"
            else "지급보증/대부 과반"
        )
        ...
        val effectivePropertyExcess = if (delinquentDays < 90 && guaranteeDebtMan > 0) propertyExcessWithGuarantee else longTermPropertyExcess
        ...
        // 최종 장기 총액 (단순유리 비교용)
        val finalLongTermTotalMonths = if (longTermUseMonths && longTermDisplayMonths > 0) longTermDisplayMonths else finalYear * 12
        val finalLongTermTotal = if (finalMonthly > 0 && finalLongTermTotalMonths > 0) finalMonthly * finalLongTermTotalMonths else 0
        ...
        if (studentLoanRatio >= 50 && !longTermDebtOverLimit) {
        ...
        } else if (income <= 100 && !hasStock && !hasCrypto && !hasGambling && originalNetProperty <= 0 && (age >= 65 || hasDisability)) {
            diagnosis = "파산"
            ...
            // 압류 → 장기(개인회생)로 압류 해제 가능 → 회워, 장기불가 or 단기불가면 방생
            val reallyShortTermBlocked = shortTermBlockedByDischarge || (effectiveShortTermBlocked && shortTermDebtOverLimit)
            diagnosis = if (!reallyShortTermBlocked && !longTermFullyBlocked) "회워" else "방생"
        } else if (isBangsaeng) {
            ...
            if (!effectiveShortTermBlocked && !longTermDebtOverLimit && shortTermTotal > 0 && saeTotalPayment - shortTermTotal > 1000 && finalLongTermTotal - shortTermTotal > 1000) {
                diagnosis = "단순유리"
            ...
            diagnosis = when {
                isBangsaeng -> "방생"
                !effectiveShortTermBlocked && !longTermDebtOverLimit && shortTermTotal > 0 && finalLongTermTotal - shortTermTotal > 1000 -> "단순유리"
                !effectiveShortTermBlocked -> if (dischargeWithin5Years || hasHfcMortgage) "워유워" else "회워"
                else -> when {
                    delinquentDays >= 90 -> if (dischargeWithin5Years || hasHfcMortgage) "워유워" else "회워"
                    delinquentDays >= 30 -> if (hoeBlocked) "프유워" else "프회워"
            ...
            diagnosis = when {
                ...
                !effectiveShortTermBlocked -> if (hoeBlocked) "워" else "회워"
                    ...
                    delinquentDays >= 30 -> if (hoeBlocked) "프워" else "프회워"
                    else -> if (hoeBlocked) "신워" else "신회워"
                }
            }
        ...
        if (recentDebtRatio >= 30 && recentDebtEntries.isNotEmpty() && targetDebt > 0 && !skipRecentDateCalc && (isSaeDiagnosis || !(effectiveShortTermBlocked && longTermFullyBlocked))) {
            ...
            for ((date, amountChon) in recentDebtEntries) {
                ...
                if (newRatio < 30) {
                    ...
                    val immediateNote = if (!isSaeDiagnosis && !effectiveShortTermBlocked && !isIncomeEstimated && !longTermDebtOverLimit && shortTermTotal > 0 && finalLongTermTotal - shortTermTotal > 1000) {
                        ", 단순 바로 가능"
                    } else {
                        ""  // 단순유리 아니면 메인 진단(장기)에서 처리
                    }
        ...
        if (diagnosis == "단순유리" && recentDebtRatio >= 30 && recentDebtEntries.isNotEmpty() && targetDebt > 0) {
            ...
            var remainingManDansun = recentDebtMan
            for ((date, amountChon) in recentDebtEntries) {
                val amountMan = (amountChon + 5) / 10
                remainingManDansun -= amountMan
                if (remainingManDansun < 0) remainingManDansun = 0
                val newRatio = remainingManDansun.toDouble() / targetDebt * 100
                if (newRatio < 30) {
                    val thresholdCal = date.clone() as Calendar
                    thresholdCal.add(Calendar.MONTH, 6)
                    val now = Calendar.getInstance()
                    val months = maxOf((thresholdCal.get(Calendar.YEAR) - now.get(Calendar.YEAR)) * 12 + (thresholdCal.get(Calendar.MONTH) - now.get(Calendar.MONTH)), 1)
                    diagnosis = "${months}개월 이후 단순 유리"
                    diagnosisNote = ""
        ...
        // 새새 + 6개월 30% 이상 대기 중 → 차량 처분 필요면 "차량 처분 후 바로 가능", 아니면 "회새 가능"
        if ((diagnosis == "새새" || diagnosis == "새" || diagnosis == "회새" || diagnosis.startsWith("새새 ") || diagnosis.startsWith("새 ") || diagnosis.startsWith("회새 ")) && diagnosis.contains("이후 가능")) {
            if (needsCarDisposal && !diagnosis.contains("차량 처분")) {
        ...
        // 차량 처분 필요 + 새출발 가능 + 회생 가능 → 차량 처분 후 바로 가능 + 차량 처분 안되면 회새 가능
        if ((needsCarDisposal || longTermCarBlockedEffective) && canApplySae && !hoeBlocked && !diagnosis.contains("회새")) {
        ...
        val childSupportSuffix = if (childSupportReceiving && childSupportAmount > 0) " (양육비+${childSupportAmount}만 포함)"
            else if (childSupportDeduction > 0) " (양육비-${childSupportDeduction}만)"
            else ""
        binding.card.text = "[소득] ${income}만${incomeSuffix}${childSupportSuffix}"
        ...
        binding.money.text = "[재산] ${formatToEok(originalNetProperty)}$registrySplitSuffix"
        ...
        val longTermDiagLabel = if (longTermFullyBlocked || longTermDebtOverLimit) "" else {
            var label = if (!canDeferment) {
                ...
                when {
                    delinquentDays >= 90 -> if (dischargeEndSoon) "워유회" else if (dischargeWithin5Years || hasHfcMortgage) "워유워" else "회워"
                    delinquentDays >= 30 -> if (dischargeEndSoon) "프유회" else "프유워"
                    else -> if (dischargeEndSoon) "신유회" else "신유워"
                }
            }
        ...
        if (canApplySae && saeTotalPayment > 0) {
            val saeBugyeol = netProperty - targetDebt >= 10000
        ...
        if (diagnosis != "방생") {
            ...
            val isLongTermBugyeol = (repaymentRate == 100 && originalTargetDebt > 4000 && !diagnosis.startsWith("단순")) || daebuRatio > 50 || (repaymentRate == 100 && majorCreditorRatio >= 50 && !diagnosis.startsWith("단순"))
        ...
        var adjustedLtLabel = longTermDiagLabel
        if (diagnosis.contains("회워")) {
            val paymentCount = when {
                fourMonthDebt >= 5000 -> 6
                threeMonthDebt >= 5000 -> 6
                threeMonthDebt >= 4000 -> 5
                twoMonthDebt >= 5000 -> 6
                twoMonthDebt >= 4000 -> 5
                twoMonthDebt >= 1000 -> 4
                else -> 0  // 바로 회워 가능
            }
            val paymentNote = if (paymentCount > 0) {
                val targetMonth = (Calendar.getInstance().get(Calendar.MONTH) + paymentCount - 1) % 12 + 1
                "${targetMonth}월까지 납부 후 회워"
            } else ""
    ...
    private fun calculateClientDiagnosis(
        ...
        if (isClientMode) {
            ...
            val isClientBugyeol = (repaymentRate == 100 && originalTargetDebt > 4000 && !diagnosis.startsWith("단순")) || daebuRatioClient > 50 || (repaymentRate == 100 && majorCreditorRatio >= 50 && !diagnosis.startsWith("단순"))
            if (isClientBugyeol) {
