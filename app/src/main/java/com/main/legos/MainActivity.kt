        ...
        fun processHwpLine(rawLine: String) {
            ...
            if (loanYear > 0 && inDebtSection && !inSpecialNotesSection && !inPropertySection && !inRegionField) {
                ...
                if (debtAmount > 0) {
                    ...
                    val isCreditByHwangsang = hasHwangsangSeqHeader && rowSeqNum > 0 && (excludedSeqNumbers.isNotEmpty() || loanCatCreditCreditorNames.isNotEmpty()) && !excludedSeqNumbers.contains(rowSeqNum) && !isInsurancePolicyLoan && !isGuaranteeLoan && !isYejeokgeumDambo
...
// 남양주 과밀억제권역 해당 지역 (이것만 포함)
    ...
    private fun calcLongTermValues(
        ...
        if (income >= 300 && householdForShinbok == 1 && parentCount > 0 && finalMonthly > 0 && totalPayment > 0) {
            ...
            var newYear = ((newMonths + 11) / 12).coerceIn(3, maxYears)
            // 년수 cap된 경우 월변제금 재계산
